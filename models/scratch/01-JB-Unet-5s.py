...
if os.path.isdir(model_andor_weight_path) & True:
    #unet = keras.models.load_model(model_andor_weight_path+'model_checkpoint_unet3_e03_base.h5')
    unet = keras.models.load_model(model_andor_weight_path+'model_unet6.h5')
    #unet.load_weights(model_andor_weight_path+'0504/weights_unet5_e04.h5')
    unet.summary()
...
unet.save(model_andor_weight_path+"model_unet6_base.h5")
# define callbacks for learning rate scheduling and best checkpoints saving

callbacks = [
    keras.callbacks.ModelCheckpoint(filepath = model_andor_weight_path+'weights_unet5_e{epoch:02d}.h5'
                                    ,save_freq = 'epoch'
                                    ,verbose = 1
...
csv_logger = CSVLogger(model_andor_weight_path+"model_history_log.csv", append=True)
...
history = unet.fit(
    train_generator, 
    steps_per_epoch= 91 ,#len(train_generator)//2, 
    epochs=EPOCHS, 
    callbacks=[callbacks, earlystop ,csv_logger], 
...
unet.save(model_andor_weight_path+"model_unet6_trained.h5")

with open(model_andor_weight_path, 'wb') as file_pi:
