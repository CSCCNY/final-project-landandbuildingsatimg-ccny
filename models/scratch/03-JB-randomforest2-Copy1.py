...
def main(image_dir, label_dir, classifier, output_model):
    ...
    train_len = len(image_list) // 5
    pool = multiprocessing.Pool()
    pool = multiprocessing.Pool(processes=4)
    image_list_inputs = [(image_list[:train_len] ,label_list[:train_len])
                         ,(image_list[train_len:train_len*2] ,label_list[train_len:train_len*2])
                         ,(image_list[train_len*2:train_len*3] ,label_list[train_len*2:train_len*3])
                         ,(image_list[train_len*3:train_len*4] ,label_list[train_len*3:train_len*4])
                         ,(image_list[-train_len:] ,label_list[-train_len:])
                        ]
